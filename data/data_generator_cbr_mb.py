...
parser.add_argument("--input-dir", type=str,
                    default="../gnnet-ch23-dataset-cbr-mb")
parser.add_argument("--output-dir", type=str,
                    default="../data_modify/data_cbr_mb_ipg_onRate")
...
def _get_network_decomposition(sample: Sample) -> Tuple[dict, list]:
    ...
    for src, dst in filter(
        ...
        for local_flow_id in range(len(traffic_matrix[src, dst]["Flows"])):
            ...
            # Obtain the time gap of sending the packet
            ipg = packet_timestamps[1:] - packet_timestamps[:-1]
            # For the case that the length of the IPG array is 0, assign it a value of [0]
            if(len(ipg) == 0):
                ipg = np.array([0])
            ipg_var = np.var(ipg)
            ipg_mean = np.mean(ipg)
            # Extract features based on the type of flow
