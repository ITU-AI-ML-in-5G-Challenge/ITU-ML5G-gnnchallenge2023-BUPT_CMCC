...
parser.add_argument("--input-dir", type=str,
                    default="/home/dyj/GNNetworkingChallenge/gnnet-ch23-test-dataset")
parser.add_argument("--output-dir", type=str,
                    default="../data/data_test_separate")
...
def _get_network_decomposition_cbr_mb(sample: Sample) -> Tuple[dict, list]:
    ...
    for src, dst in filter(
        ...
        for local_flow_id in range(len(traffic_matrix[src, dst]["Flows"])):
            ...
            # new change
            packet_timestamps = np.array(
                [float(x[0]) for x in flow_packet_info])
            ...
            if(flow["TimeDist"] == TimeDist.CBR_T):
                rate = flow['TimeDistParams']['Rate']
                ibg = 0
            else:
                ibg = flow['TimeDistParams']['IBG']
                rate = flow['TimeDistParams']['On_Rate']
            # new change
    ...
    sample = (
        {
            ...
            "flow_on_rate": np.expand_dims(
                [flow["on_rate"] for flow in ordered_flows],axis=1
            ),
            "flow_ibg": np.expand_dims(
...
def _generator(
    ...
    for sample in iter(tool):
        ret = _get_network_decomposition_cbr_mb(sample)
        print (f"sample{index} has been scanned！")
        index+=1
    ...
    signature = (
        {
            ...
            "flow_on_rate": tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
            "flow_ibg": tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
            # new change
        },
